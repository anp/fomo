...
mod windows_tests {
  ...
  fn shutdown() {
    ...
    while watchers_shutdown != dir_count && start.elapsed() < Duration::from_millis(TIMEOUT_MS) {
      ...
      // don't burn cpu, can take some time for completion events to fire
  ...
  // you use task manager to watch the memory; it will fluctuate a bit, but should not leak
  // overall
  fn memtest_manual() {
    let mut i = 0;
    loop {
      ...
      let d = TempDir::new("rsnotifytest").unwrap();
      {
        ...
        let mut w = ReadDirectoryChangesWatcher::create(tx, meta_tx).unwrap();
        w.watch(d.path(), RecursiveMode::Recursive).unwrap();
        // this should make us run pretty hot but not insane
        thread::sleep(Duration::from_millis(1));
      }
      wait_for_disconnect(&rx);
      i += 1;
