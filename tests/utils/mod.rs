...
pub trait TestHelpers {
  /// Return path relative to the TempDir. Directory separator must be a forward slash,
  /// and will be converted to the platform's native separator.
  fn mkpath(&self, p: &str) -> PathBuf;
  /// Create file or directory. Directories must contain the phrase "dir" otherwise they
  /// will be interpreted as files.
  fn create(&self, p: &str);
  /// Create all files and directories in the `paths` list. Directories must contain the
  /// phrase "dir" otherwise they will be interpreted as files.
        ...
        match (&($left), &($right1), &($right2)) {
            (left_val, right1_val, right2_val) => {
                if *left_val != *right1_val && *left_val != *right2_val {
                    panic!(
                      "assert failed: `(l != r1 or r2)` (left: `{:?}`, r1: `{:?}`, r2: `{:?}`)",
                      left_val, right1_val, right2_val)
                }
            }
        }
