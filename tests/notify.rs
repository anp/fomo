...
fn test_inflate_events() {
  assert_eq!(inflate_events(vec![ (PathBuf::from("file1"), op::CREATE, None),
                                  (PathBuf::from("file1"), op::WRITE, None) ]),
             vec![ (PathBuf::from("file1"), op::CREATE | op::WRITE, None) ]);

  assert_eq!(inflate_events(vec![ (PathBuf::from("file1"), op::RENAME, Some(1)),
                                  (PathBuf::from("file1"), op::RENAME, Some(2)) ]),
             vec![ (PathBuf::from("file1"), op::RENAME, Some(1)),
                   (PathBuf::from("file1"), op::RENAME, Some(2)) ]);
}
...
fn write_file() {
  ...
  if cfg!(target_os = "macos") {
    assert_eq!(inflate_events(recv_events(&rx)),
               vec![ (tdir.mkpath("file1"), op::CREATE | op::WRITE, None) /* excessive
                                                                           * create event */ ]);
  } else if cfg!(target_os = "windows") {
    assert_eq!(recv_events(&rx),
...
fn modify_file() {
  ...
  if cfg!(target_os = "windows") {
    ...
    assert_eq!(inflate_events(recv_events(&rx)),
               vec![ (tdir.mkpath("file1"), op::CHMOD | op::CREATE, None) ]);
...
fn delete_file() {
  ...
  if cfg!(target_os = "macos") {
    assert_eq!(inflate_events(recv_events(&rx)),
               vec![ (tdir.mkpath("file1"), op::CREATE | op::REMOVE, None) /* excessive
                                                                            * create event */ ]);
  } else {
    assert_eq!(recv_events(&rx),
               vec![ (tdir.mkpath("file1"), op::REMOVE, None) ]);
  }
}
...
fn rename_file() {
  ...
  if cfg!(target_os = "macos") {
    ...
    assert_eq!(actual,
               ...
               vec![ (tdir.mkpath("file1a"), op::CREATE | op::RENAME, Some(cookies[ 0 ])),
    ...
    assert_eq!(cookies.len(), 1);
    assert_eq!(actual,
               vec![ (tdir.mkpath("file1a"), op::RENAME, Some(cookies[ 0 ])),
                     (tdir.mkpath("file1b"), op::RENAME, Some(cookies[ 0 ])) ]);
  }
}
...
fn move_out_create_file() {
  ...
  if cfg!(target_os = "macos") {
    assert_eq!(inflate_events(recv_events(&rx)),
               ...
               vec![ (tdir.mkpath("watch_dir/file1"), op::CREATE | op::RENAME, None) ]);
    panic!("move event should be a remove event; fsevent conflates rename and create events");
  } else if cfg!(target_os = "linux") {
    assert_eq!(recv_events(&rx),
               vec![ (tdir.mkpath("watch_dir/file1"), op::REMOVE, None),
...
fn create_write_modify_file() {
  ...
  if cfg!(target_os = "windows") {
    ...
    assert_eq!(inflate_events(recv_events(&rx)),
               vec![ (tdir.mkpath("file1"), op::CHMOD | op::CREATE | op::WRITE, None) ]);
    panic!("macos cannot distinguish between chmod and create");
  } else if cfg!(target_os = "linux") {
    assert_eq!(recv_events(&rx),
               vec![ (tdir.mkpath("file1"), op::CREATE, None),
...
fn create_rename_overwrite_file() {
  ...
  if cfg!(target_os = "windows") {
    ...
    assert_eq!(actual,
               vec![ (tdir.mkpath("file1a"), op::CREATE, None),
                     (tdir.mkpath("file1a"), op::REMOVE, None),
                     (tdir.mkpath("file1b"), op::WRITE, None) ]);
  } else if cfg!(target_os = "macos") {
    assert_eq!(inflate_events(actual),
               vec![ (tdir.mkpath("file1a"), op::CREATE | op::RENAME, None),
...
fn create_directory() {
  ...
  assert_eq!(actual, vec![ (tdir.mkpath("dir1"), op::CREATE, None) ]);
}
...
fn modify_directory() {
  ...
  if cfg!(target_os = "windows") {
    assert_eq!(recv_events(&rx),
               vec![ (tdir.mkpath("dir1"), op::WRITE, None) ]);
    panic!("windows cannot distinguish between chmod and write");
  } else if cfg!(target_os = "macos") {
    assert_eq!(inflate_events(recv_events(&rx)),
               vec![ (tdir.mkpath("dir1"), op::CHMOD | op::CREATE, None) ]);
    panic!("macos cannot distinguish between chmod and create");
  } else {
    // TODO: emit chmod event only once
    assert_eq!(recv_events(&rx),
               vec![ (tdir.mkpath("dir1"), op::CHMOD, None),
...
fn delete_directory() {
  ...
  if cfg!(target_os = "macos") {
    ...
    assert_eq!(recv_events(&rx),
               vec![ (tdir.mkpath("dir1"), op::REMOVE, None) ]);
  }
}
...
fn rename_directory() {
  ...
  if cfg!(target_os = "macos") {
    ...
    assert_eq!(actual,
               // excessive create event
               vec![ (tdir.mkpath("dir1a"), op::CREATE | op::RENAME, Some(cookies[ 0 ])),
    ...
    assert_eq!(cookies.len(), 1);
    assert_eq!(actual,
               vec![ (tdir.mkpath("dir1a"), op::RENAME, Some(cookies[ 0 ])),
                     (tdir.mkpath("dir1b"), op::RENAME, Some(cookies[ 0 ])) ]);
  }
}
...
fn move_out_create_directory() {
  ...
  if cfg!(target_os = "macos") {
    assert_eq!(inflate_events(recv_events(&rx)),
               // fsevent interprets a move_out as a rename event
               vec![ (tdir.mkpath("watch_dir/dir1"), op::CREATE | op::RENAME, None) ]);
    panic!("move event should be a remove event; fsevent conflates rename and create events");
  } else {
    assert_eq!(recv_events(&rx),
               vec![ (tdir.mkpath("watch_dir/dir1"), op::REMOVE, None),
...
fn create_rename_overwrite_directory() {
  ...
  if cfg!(target_os = "macos") {
    assert_eq!(inflate_events(recv_events(&rx)),
               vec![ (tdir.mkpath("dir1a"), op::CREATE | op::RENAME, None),
    ...
    assert_eq!(cookies.len(), 1);
    assert_eq!(actual,
               vec![ (tdir.mkpath("dir1a"), op::CREATE, None),
