  ...
  // parallel
  {
    ...
    if cfg!(target_os = "macos") {
      assert_eq!(recv_events_simple(&rx),
                 vec![ (tdir.mkpath("dir1/file1"), op::CREATE, None) /* fsevents always
                                                                      * returns canonicalized
                                                                      * paths */ ]);
    } else {
      assert_eq!(recv_events_simple(&rx),
                 vec![ (tdir.path().join("dir1/file1"), op::CREATE, None) ]);
    }
  }
  {
    // watch_relative_file
    ...
    if cfg!(target_os = "macos") {
      assert_eq!(recv_events_simple(&rx),
                 vec![ (tdir.mkpath("file1"), op::WRITE, None) /* fsevents always returns
                                                                * canonicalized paths */ ]);
    } else {
      assert_eq!(recv_events_simple(&rx),
                 vec![ (tdir.path().join("file1"), op::WRITE, None) ]);
    }
  }
...
fn watch_absolute_directory() {
  ...
  if cfg!(target_os = "macos") {
    ...
    assert_eq!(recv_events_simple(&rx),
               vec![ (watch_path.join("file1"), op::CREATE, None) ]);
  }
}
