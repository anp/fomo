...
fn test_watcher_send() {
  ...
  thread::spawn(move || { watcher.watch(".", RecursiveMode::Recursive).unwrap(); })
    .join()
    .unwrap();
}
...
#[test]
fn watch_relative() {
  // both of the following tests set the same environment variable, so they must not run in
  // parallel
  {
    ...
    let mut watcher: RecommendedWatcher = Watcher::new_raw(tx)
      .expect("failed to create recommended watcher");
...
fn watch_recursive_create_directory() {
  ...
  if cfg!(target_os = "linux") {
    assert_eq!(actual,
               vec![ (tdir.mkpath("dir1"), op::CREATE, None),
...
fn watch_recursive_move() {
  ...
  if cfg!(target_os = "macos") {
    ...
    assert_eq!(actual,
               vec![ (tdir.mkpath("dir1a/file1"), op::CREATE, None),
                     // excessive create event
                     (tdir.mkpath("dir1a"), op::CREATE | op::RENAME, Some(cookies[ 0 ])),
...
fn watch_recursive_move_in() {
  ...
  if cfg!(target_os = "macos") {
    assert_eq!(actual,
               vec![ (tdir.mkpath("watch_dir/dir1b"), op::RENAME, None), /* fsevent interprets
                                                                          * a move_to as a
                                                                          * rename event */
    ...
    assert_eq!(actual,
               vec![ (tdir.mkpath("watch_dir/dir1b"), op::CREATE, None),
                     (tdir.mkpath("watch_dir/dir1b/dir1/file1"), op::CREATE, None),
...
fn watch_recursive_move_out() {
  ...
  if cfg!(target_os = "macos") {
    assert_eq!(actual,
               vec![ (tdir.mkpath("watch_dir/dir1a/dir1/file1"), op::CREATE, None),
                     // excessive create event, fsevent interprets a move_out as a rename event
                     (tdir.mkpath("watch_dir/dir1a"), op::CREATE | op::RENAME, None) ]);
...
fn watch_nonrecursive() {
  ...
  if cfg!(target_os = "linux") {
    assert_eq!(recv_events(&rx),
               vec![ (tdir.mkpath("dir2"), op::CREATE, None),
...
fn watch_file() {
  ...
  if cfg!(target_os = "macos") {
    assert_eq!(recv_events(&rx),
               vec![ (tdir.mkpath("file1"), op::CREATE | op::WRITE, None) /* excessive write
                                                                           * create */ ]);
  } else if cfg!(target_os = "linux") {
    assert_eq!(recv_events(&rx),
               vec![ (tdir.mkpath("file1"), op::WRITE, None),
...
fn poll_watch_recursive_create_directory() {
  ...
  assert_eq!(actual,
             vec![ (tdir.mkpath("."), op::WRITE, None), // parent directory gets modified
...
fn poll_watch_recursive_move() {
  ...
  assert_eq!(actual,
             vec![ (tdir.mkpath("dir1a"), op::WRITE, None), // parent directory gets modified
...
fn poll_watch_recursive_move_in() {
  ...
  if cfg!(target_os = "windows") {
    assert_eq!(actual,
               vec![ (tdir.mkpath("watch_dir"), op::WRITE, None), /* parent directory gets
                                                                   * modified */
                     (tdir.mkpath("watch_dir/dir1b"), op::CREATE, None),
                     (tdir.mkpath("watch_dir/dir1b/dir1"), op::CREATE, None),
                     (tdir.mkpath("watch_dir/dir1b/dir1"), op::WRITE, None), /* extra write
                                                                              * event */
                     (tdir.mkpath("watch_dir/dir1b/dir1/file1"), op::CREATE, None) ]);
  } else {
    assert_eq!(actual,
...
fn poll_watch_recursive_move_out() {
  ...
  assert_eq!(actual,
             ...
             vec![ (tdir.mkpath("watch_dir/dir1a/dir1"), op::WRITE, None),
...
fn watch_nonexisting() {
  ...
  if cfg!(target_os = "macos") {
    assert_eq!(inflate_events(recv_events(&rx)),
               vec![ (tdir1.mkpath("file1"), op::CREATE, None) ]);
...
fn self_delete_file() {
  ...
  if cfg!(target_os = "windows") {
    ...
    assert_eq!(actual,
               vec![ (tdir.mkpath("file1"), op::CREATE | op::REMOVE, None) ]);
  } else {
    assert_eq!(actual,
               vec![ (tdir.mkpath("file1"), op::CHMOD, None),
...
fn self_delete_directory() {
  ...
  if cfg!(target_os = "windows") {
    ...
    assert_eq!(actual,
               vec![ (tdir.mkpath("dir1"), op::CREATE | op::REMOVE, None) ]);
  } else {
    assert_eq!(actual, vec![ (tdir.mkpath("dir1"), op::REMOVE, None) ]);
  }
  ...
  if cfg!(target_os = "macos") {
    assert_eq!(actual,
               vec![ (tdir.mkpath("dir1"), op::CREATE | op::REMOVE, None) /* excessive
                                                                           * remove event */ ]);
  } else {
...
fn self_rename_file() {
  ...
  if cfg!(target_os = "macos") {
    assert_eq!(actual,
               vec![ (tdir.mkpath("file1"), op::CREATE | op::RENAME, None) /* excessive
                                                                            * create event */ ]);
  } else {
    assert_eq!(actual, vec![ (tdir.mkpath("file1"), op::RENAME, None) ]);
  }
  ...
  if cfg!(target_os = "windows") {
    ...
    assert_eq!(actual,
               vec![ (tdir.mkpath("file1"), op::WRITE, None), // path doesn't get updated
                     (tdir.mkpath("file1"), op::CLOSE_WRITE, None) /* path doesn't get
                                                                    * updated */ ]);
  } else {
    assert_eq!(actual,
               vec![ (tdir.mkpath("file1"), op::WRITE, None) /* path doesn't get updated */ ]);
  ...
  if cfg!(target_os = "macos") {
    ...
    assert_eq!(recv_events(&rx),
               ...
               vec![ (tdir.mkpath("file1"), op::CREATE | op::RENAME, None) ]);
  } else {
    assert_eq!(recv_events(&rx), vec![]);
  }

  // use old path to unwatch
  watcher.unwatch(&tdir.mkpath("file1")).expect("failed to unwatch file");
...
fn self_rename_directory() {
  ...
  if cfg!(target_os = "windows") {
    ...
    assert_eq!(actual,
               vec![ (tdir.mkpath("dir1"), op::CREATE | op::RENAME, None) /* excessive
                                                                           * create event */ ]);
  } else {
    assert_eq!(actual, vec![ (tdir.mkpath("dir1"), op::RENAME, None) ]);
  }
  ...
  if cfg!(target_os = "macos") {
    ...
    assert_eq!(actual,
               vec![ (tdir.mkpath("dir1/file1"), op::CREATE, None), // path doesn't get updated
                     (tdir.mkpath("dir1/file1"), op::CLOSE_WRITE, None) ]);
  } else {
    assert_eq!(actual,
               vec![ (tdir.mkpath("dir1/file1"), op::CREATE, None) /* path doesn't get
                                                                    * updated */ ]);
  }
...
fn parent_rename_file() {
  ...
  let mut watcher: RecommendedWatcher = Watcher::new_raw(tx)
    .expect("failed to create recommended watcher");
  watcher.watch(&tdir.mkpath("dir1/file1"), RecursiveMode::Recursive)
  ...
  if cfg!(target_os = "macos") {
    ...
    assert_eq!(actual,
               vec![ (tdir.mkpath("dir1/file1"), op::WRITE, None), // path doesn't get updated
                     (tdir.mkpath("dir1/file1"), op::CLOSE_WRITE, None) ]);
  } else {
    assert_eq!(actual,
               vec![ (tdir.mkpath("dir1/file1"), op::WRITE, None) /* path doesn't get updated */ ]);
  ...
  if cfg!(target_os = "macos") {
    // macos doesn't watch files, but paths
    assert_eq!(recv_events(&rx),
               vec![ (tdir.mkpath("dir1/file1"), op::CREATE, None) ]);
  } else {
    assert_eq!(recv_events(&rx), vec![]);
  }
...
fn parent_rename_directory() {
  ...
  let mut watcher: RecommendedWatcher = Watcher::new_raw(tx)
    .expect("failed to create recommended watcher");
  watcher.watch(&tdir.mkpath("dir1/watch_dir"), RecursiveMode::Recursive)
    .expect("failed to watch directory");
  ...
  if cfg!(target_os = "macos") {
    ...
    assert_eq!(actual,
               ...
               vec![ (tdir.mkpath("dir1/watch_dir/file1"), op::CREATE, None),
                     // path doesn't get updated
                     (tdir.mkpath("dir1/watch_dir/file1"), op::CLOSE_WRITE, None) ]);
  } else {
    assert_eq!(actual,
               // path doesn't get updated
               vec![ (tdir.mkpath("dir1/watch_dir/file1"), op::CREATE, None) ]);
  ...
  if cfg!(target_os = "macos") {
    // macos doesn't watch files, but paths
    assert_eq!(actual,
               vec![ (tdir.mkpath("dir1/watch_dir"), op::CREATE, None) ]);
  } else {
    assert_eq!(actual, vec![]);
  }
  // use old path to unwatch
