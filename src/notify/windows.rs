...
use winapi::{ERROR_OPERATION_ABORTED, FILE_NOTIFY_INFORMATION, HANDLE, INFINITE,
             INVALID_HANDLE_VALUE, LPOVERLAPPED, OVERLAPPED, TRUE, WAIT_OBJECT_0, fileapi, winbase,
             winnt};
...
use super::{DebouncedEvent, Error, Op, RawEvent, RecursiveMode, Result, Watcher, op};
...
impl ReadDirectoryChangesServer {
  ...
  fn add_watch(&mut self, path: PathBuf, is_recursive: bool) -> Result<PathBuf> {
    ...
    unsafe {
      ...
      if handle == INVALID_HANDLE_VALUE {
        let err = if watching_file {
          Err(Error::Generic("You attempted to watch a single file, but parent directory could \
                              not be opened."
            .to_owned()))
        } else {
    ...
    let semaphore = unsafe { kernel32::CreateSemaphoreW(ptr::null_mut(), 0, 1, ptr::null_mut()) };
...
fn start_read(rd: &ReadData, event_tx: Arc<Mutex<EventTx>>, handle: HANDLE) {
  ...
  let flags = winnt::FILE_NOTIFY_CHANGE_FILE_NAME | winnt::FILE_NOTIFY_CHANGE_DIR_NAME |
              ...
              winnt::FILE_NOTIFY_CHANGE_SIZE | winnt::FILE_NOTIFY_CHANGE_LAST_WRITE |
              winnt::FILE_NOTIFY_CHANGE_CREATION | winnt::FILE_NOTIFY_CHANGE_SECURITY;
  ...
  unsafe {
    ...
    // This is using an asynchronous call with a completion routine for receiving
    // notifications
...
unsafe extern "system" fn handle_event(
  ...
  if let Ok(mut event_tx) = event_tx_lock {
    ...
    // string as its last member.  Each struct contains an offset for getting the next entry
    // in
    // the buffer.
...
impl ReadDirectoryChangesWatcher {
  pub fn create(
    ...
    let wakeup_sem = unsafe { kernel32::CreateSemaphoreW(ptr::null_mut(), 0, 1, ptr::null_mut()) };
  ...
  fn wakeup_server(&mut self) {
    // breaks the server out of its wait state.  right now this is really just an
    // optimization,
            ...
            match ack_res {
              ...
              Ok(ack_pb) => {
                if pb.as_path() != ack_pb.as_path() {
                  Err(Error::Generic(format!("Expected ack for {:?} but got ack for {:?}",
                                             pb,
                                             ack_pb)))
                } else {
                  Ok(())
...
// Because all public methods are `&mut self` it's also perfectly safe to share
// references.
