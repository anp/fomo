...
//! # Examples
//!
//! Notify provides two APIs. The default API _debounces_ events (if the backend reports
//! two
//! similar events in close succession, Notify will only report one). The raw API emits
//! file
...
//! extern crate fomo;
//!
//! use fomo::notify::{Watcher, RecursiveMode, watcher};
...
//! [`DebouncedEvent`](enum.DebouncedEvent.html) documentation. But in order to
//! understand the
//! subtleties of the event delivery, you should read the [`op`](op/index.html)
//! documentation as
//! well.
//!
//! ## Raw API
...
//! extern crate fomo;
//!
//! use fomo::notify::{Watcher, RecursiveMode, RawEvent, raw_watcher};
//! use std::sync::mpsc::channel;
//!
//! fn main() {
//!     // Create a channel to receive the events.
...
//! ```
//!
//! The event structure is described in the [`RawEvent`](struct.RawEvent.html)
//! documentation,
//! all possible operations delivered in an event are described in the
//! [`op`](op/index.html)
//! documentation.
...
///
/// `notify` aims to provide unified behavior across platforms. This however is not
/// always possible
/// due to the underlying technology of the various operating systems. So there are some
/// issues
...
/// __Linux, OS X__
///
/// On Linux and OS X the `CHMOD` event is emitted whenever attributes or extended
/// attributes
/// change.
///
/// __Windows__
///
/// On Windows a `WRITE` event is emitted when attributes change. This makes it
/// impossible to
...
/// # Close-Write
///
/// A `CLOSE_WRITE` event is emitted whenever a file that was opened for writing has been
/// closed.
///
...
///
/// Upon receiving a `Create` event for a directory, it is necessary to scan the newly
/// created
/// directory for contents. The directory can contain files or directories if those
/// contents were
/// created before the directory could be watched, or if the directory was moved into the
/// watched
/// directory.
///
/// # Remove
...
/// directory itself is removed. The behavior after the remove differs between platforms
/// though.
///
/// __Linux__
///
/// When a watched file or directory is removed, its watch gets destroyed and no new
/// events will be
/// sent.
///
/// __Windows__
...
///
/// When watching a single file on Windows, the file path will continue to be watched
/// until either
/// the watch is removed by the API user or the parent directory gets removed.
///
/// When watching a directory on Windows, the watch will get destroyed and no new events
/// will be
/// sent.
///
/// __OS X__
///
/// While Linux and Windows monitor "inodes", OS X monitors "paths". So a watch stays
/// active even
/// after the watched file or directory has been removed and it will emit events in case
/// a new file
...
/// # Rename
///
/// A `RENAME` event is emitted whenever a file or directory has been renamed or moved to
/// a
/// different directory.
///
...
/// __Linux, Windows__
///
/// A rename with both the source and the destination path inside a watched directory
/// produces two
/// `RENAME` events. The first event contains the source path, the second contains the
/// destination
/// path. Both events share the same cookie.
///
/// A rename that originates inside of a watched directory but ends outside of a watched
/// directory
/// produces a `DELETE` event.
///
/// A rename that originates outside of a watched directory and ends inside of a watched
/// directory
/// produces a `CREATE` event.
///
/// __OS X__
///
/// A `RENAME` event is produced whenever a file or directory is moved. This includes
/// moves within
/// the watched directory as well as moves into or out of the watched directory. It is up
/// to the
/// API user to determine what exactly happened. Usually when a move within a watched
/// directory
/// occurs, the cookie is set for both connected events. This can however fail eg. if a
/// file gets
/// renamed multiple times without a delay (test `fsevents_rename_rename_file_0`). So in
/// some cases
/// rename cannot be caught properly but would be interpreted as a sequence of events
/// where a file
/// or directory is moved out of the watched directory and a different file or directory
/// is moved
/// in.
///
...
/// directory itself is renamed. The behavior after the rename differs between platforms
/// though.
/// Depending on the platform either the moved file or directory will continue to be
/// watched or the
/// old path. If the moved file or directory will continue to be watched, the paths of
/// emitted
/// events will still be prefixed with the old path though.
///
/// __Linux__
///
/// Linux will continue to watch the moved file or directory. Events will contain paths
/// prefixed
/// with the old path.
///
/// __Windows__
///
/// Currently there is no event emitted when a watched directory is renamed. But the
/// directory will
/// continue to be watched and events will contain paths prefixed with the old path.
///
/// When renaming a watched file, a `RENAME` event is emitted but the old path will
/// continue to be
/// watched.
///
/// __OS X__
...
///
/// Currently no event will be emitted when any parent directory of the watched file or
/// directory
/// is renamed. Depending on the platform either the moved file or directory will
/// continue to be
/// watched or the old path. If the moved file or directory will continue to be watched,
/// the paths
/// of emitted events will still be prefixed with the old path though.
///
/// __Linux, Windows__
///
/// Linux and Windows will continue to watch the moved file or directory. Events will
/// contain paths
...
/// # Rescan
///
/// A `RESCAN` event indicates that an error occurred and the watched directories need to
/// be
/// rescanned. This can happen if the internal event queue has overflown and some events
/// were
...
///
/// Windows: 16384 Bytes. The actual amount of events that fit into the queue depends on
/// the length
/// of the paths.
///
///
...
pub struct RawEvent {
  ...
  ///
  /// `path` is always absolute, even if a relative path is used to watch a file or
  /// directory.
  ///
  /// On **macOS** the path is always canonicalized.
  ///
  /// Keep in mind that the path may be false if the watched file or directory or any
  /// parent
  ...
  ///
  /// When using the `PollWatcher`, `op` may be `Err` if reading meta data for the path
  /// fails.
  ///
  /// When using the `INotifyWatcher`, `op` may be `Err` if activity is detected on the
  /// file and
  ...
  ///
  /// If two consecutive `RENAME` events share the same cookie, it means that the first
  /// event
...
pub enum DebouncedEvent {
  ...
  ///
  /// If you are reading from that file, you should probably close it immediately and
  /// discard all
  ...
  /// `Create` is emitted when a file or directory has been created and no events were
  /// detected
  /// for the path within the specified time frame.
  ///
  /// `Create` events have a higher priority than `Write` and `Chmod`. These events will
  /// not be
  /// emitted if they are detected before the `Create` event has been emitted.
  Create(PathBuf),

  /// `Write` is emitted when a file has been written to and no events were detected for
  /// the path
  /// within the specified time frame.
  ///
  /// `Write` events have a higher priority than `Chmod`. `Chmod` will not be emitted if
  /// it's
  ...
  /// directory for contents. The directory can contain files or directories if those
  /// contents
  /// were created before the directory could be watched, or if the directory was moved
  /// into the
  /// watched directory.
  Write(PathBuf),

  /// `Chmod` is emitted when attributes have been changed and no events were detected
  /// for the
  /// path within the specified time frame.
  Chmod(PathBuf),

  /// `Remove` is emitted when a file or directory has been removed and no events were
  /// detected
  /// for the path within the specified time frame.
  Remove(PathBuf),

  /// `Rename` is emitted when a file or directory has been moved within a watched
  /// directory and
  ...
  /// `Rescan` is emitted immediately after a problem has been detected that makes it
  /// necessary
...
/// Indicates whether only the provided directory or its sub-directories as well should
/// be watched
#[derive(Debug)]
pub enum RecursiveMode {
  /// Watch all sub-directories as well, including directories created after installing
  /// the watch
  Recursive,
...
/// platform.
/// In addition to such event driven implementations, a polling implementation is also
/// provided
...
pub trait Watcher: Sized {
  ...
  ///
  /// For example you won't have to handle `RENAME` events yourself by piecing the two
  /// parts of
  ...
  /// something is written to a file, a `NoticeWrite` event is emitted. If no new event
  /// arrived
  /// until after the specified `delay`, a `Write` event is emitted.
  ///
  /// A practical example would be the safe-saving of a file, where a temporary file is
  /// created
  /// and written to, then only when everything has been written to that file is it
  /// renamed to
  /// overwrite the file that was meant to be saved. Instead of receiving a `CREATE`
  /// event for
  /// the temporary file, `WRITE` events to that file and a `RENAME` event from the
  /// temporary
  /// file to the file being saved, you will just receive a single `Write` event.
  ///
  /// If you use a delay of more than 30 seconds, you can avoid receiving repetitions of
  /// previous
  /// events on OS X.
  ///
  /// # Disadvantages
  ...
  ///
  /// If the `path` is a directory, `recursive_mode` will be evaluated. If
  /// `recursive_mode` is
  /// `RecursiveMode::Recursive` events will be delivered for all files in that tree.
  /// Otherwise
  /// only the directory and its immediate children will be watched.
  ///
  /// If the `path` is a file, `recursive_mode` will be ignored and events will be
  /// delivered only
  /// for the file.
  ...
  /// # Errors
  ///
  /// Returns an error in the case that `path` has not been watched or if removing the
  /// watch
  /// fails.
