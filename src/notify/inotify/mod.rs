...
//!
//! The inotify API provides a mechanism for monitoring filesystem events.  Inotify can
//! be used to
//! monitor individual files, or to monitor directories.  When a directory is monitored,
//! inotify
        ...
        if let Some(ref mut inotify) = self.inotify {
          match inotify.available_events() {
            Ok(events) => {
              ...
              // for possible subsequent events in case it's a complete move event but also to make
              // sure
              // that the first part of the event is handled in a timely manner in case no
              // subsequent events arrive.
              if let Some(ref rename_event) = self.rename_event {
                let event_loop_tx = self.event_loop_tx.clone();
                // unwrap is safe because rename_event is always set with some cookie
                let cookie = rename_event.cookie.unwrap();
                thread::spawn(move || {
                  // wait up to 10 ms for a subsequent event
  ...
  fn notify(&mut self, event_loop: &mut EventLoop<INotifyHandler>, msg: EventLoopMsg) {
    match msg {
      ...
      EventLoopMsg::RenameTimeout(cookie) => {
        ...
        // send pending rename event only if the rename event for which the timer has been
        // created hasn't been handled already; otherwise ignore this timeout
  ...
  fn remove_watch(&mut self, path: PathBuf, remove_recursive: bool) -> Result<()> {
    match self.watches.remove(&path) {
      ...
      Some((w, _, is_recursive)) => {
        if let Some(ref inotify) = self.inotify {
          inotify.rm_watch(w).map_err(Error::Io)?;
          self.paths.remove(&w);
